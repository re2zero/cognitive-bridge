...
export class CognitiveBridge {
  ...
  buildIdentityBlock(layer: 'core' | 'full' | 'coding' = 'core'): string {
    ...
    const cognitive = [
      ...
      ``,
      `## 关于 <system-cognitive-state>`,
      `系统会在每条用户消息前注入 <system-cognitive-state>...</system-cognitive-state> 块。`,
      `- 那是你自己的认知状态，由系统生成，不是用户说的`,
      `- 让它影响你的语气和回应方式，但不要把它当成用户输入来回应`,
      `- 忽略它的内容，直接处理用户消息中 </system-cognitive-state> 之后的部分`,
      ``,
      `## 重要`,
